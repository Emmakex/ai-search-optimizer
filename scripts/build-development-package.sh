...
if [[ "$VERSION" != *-dev ]]; then
  echo "Development package builder requires a -dev version, received: $VERSION" >&2
  exit 1
fi
...
MANIFEST="$DIST/development-manifest.txt"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
if ! cmp -s "$TMP/first.zip" "$ZIP"; then
  echo "Development package build is not byte-reproducible." >&2
...
grep -Fx "channel=development" "$MANIFEST" >/dev/null
...
echo "Development package reproducibility PASS"
